...
// Helper function to check Vector2 userdata
Vector2* checkVector2(lua_State* L, int index) {
    return static_cast<Vector2*>(luaL_checkudata(L, index, "Vector2"));
}

// Push a new Vector2 to Lua stack
static void pushVector2(lua_State* L, const Vector2& vec) {
...
// Vector2 tween implementation
Vector2 Vector2::tween(const Vector2& target, float t, const char* direction, const char* style) const {
    ...
    return Vector2(x + (target.x - x) * easedT, y + (target.y - y) * easedT);
}
...
int l_vector2_multiply(lua_State* L) {
    ...
    if (lua_isnumber(L, 2)) {
        ...
        pushVector2(L, a->multiply(scalar));
    } else {
        Vector2* b = checkVector2(L, 2);
        pushVector2(L, a->multiply(*b));
    }
    return 1;
}
...
int l_vector2_divide(lua_State* L) {
    ...
    if (lua_isnumber(L, 2)) {
        ...
        pushVector2(L, a->divide(scalar));
    } else {
        Vector2* b = checkVector2(L, 2);
        if (b->x == 0 || b->y == 0) luaL_error(L, "Division by zero");
        pushVector2(L, a->divide(*b));
    }
    return 1;
}
...
int l_vector2_tween(lua_State* L) {
    ...
    pushVector2(L, self->tween(*target, t, direction, style));
    return 1;
}